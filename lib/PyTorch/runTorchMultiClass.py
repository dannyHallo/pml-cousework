...
def train_model(x_train, y_train, class_weight=None, x_val=None, y_val=None,
    ...
    if x_val is not None:  # TODO: check dimensions when supplying validation data.
        ...
        train_loader = build_dataloader(
            x_train, y_train, n_channels=model.n_channels)
...
#     criterion = nn.BCELoss()  # Let's use CE loss for multiclass compatibility
    ...
    for e in range(config_pytorch.epochs):
        ...
        for batch_i, inputs in enumerate(train_loader):
            ...
            # will need to check for two-class also.
        ...
        print('all_y', 'all_y_pred', np.shape(all_y.numpy()),
              np.shape(np.argmax(all_y_pred.numpy(), axis=1)))
        train_acc = accuracy_score(
            all_y.numpy(), np.argmax(all_y_pred.numpy(), axis=1))
        ...
        if acc_metric > best_acc_metric:
            ...
            checkpoint_name = f'model_e{e}_{
                datetime.now().strftime("%Y_%m_%d_%H_%M_%S")}.pth'

            torch.save(model.state_dict(), os.path.join(
                config.model_dir, 'pytorch', checkpoint_name))
        ...
        overrun_counter += 1
        if x_val is not None:
            print('Epoch: %d, Train Loss: %.8f, Train Acc: %.8f, Val Loss: %.8f, Val Acc: %.8f, overrun_counter %i' % (
                e, train_loss/len(train_loader), train_acc, val_loss/len(val_loader), val_acc,  overrun_counter))
        else:
